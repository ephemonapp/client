...
10. **There is no pre-MLS history to import.** D-010 withdrew the legacy-intersection protocol: the messenger has no
    released clients, so pre-MLS history and its migration path are deleted rather than carried. The chat-update
    protocol survives only as the self-chat transport under D-009.
...
### Increment 9 — Remove pre-MLS support

Under D-010 there is no pre-MLS history to migrate, so this increment only deletes what that support required.

Deliverables:

- one initializing schema migration replacing the `001`–`006` chain and the old-vault import;
- removal of the pre-upgrade conversation state and the upgrade boundary;
- the chat-update protocol retained only as the self-chat transport under D-009, named accordingly;
- cleanup of now-unused pako/text dependencies and obsolete schema fields;
- updated protocol diagram and public security guarantees.

Exit criteria:

- a direct conversation is either bootstrapping MLS or is MLS, never ambiguous and never pre-upgrade;
- an active attacker or old peer cannot force a direct conversation off MLS onto the chat-update protocol;
- self-chat keeps working through the chat-update protocol, including its direction-scoped record identity;
- a fresh vault is created from one migration with no data conversion step.
...
- [x] pre-MLS history and its migration path deleted instead of migrated (D-010 supersedes D-004);
