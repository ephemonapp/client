...
/// A single group-scoped MLS client. Ephemon creates one instance per conversation, so the
/// signing key and BasicCredential are never reused to link otherwise unrelated groups.
...
impl MlsClient {
    ...
    /// Creates an add Commit and Welcome but intentionally does not merge the pending Commit.
    /// The application policy layer decides whether this client is authorized to commit and when
    /// the transport-level acknowledgement is sufficient to merge.
