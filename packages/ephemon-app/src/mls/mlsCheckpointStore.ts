...
export function createVaultMlsCheckpointStore(database: Database = getVault()): MlsCheckpointStore {
    return {
        async load(conversationId) {
            ...
            return stored === null
                ? null
                : { revision: stored.revision, bytes: stored.checkpoint, companion: stored.companion };
        },
        save(conversationId, expectedRevision, bytes, companion) {
            return database.putMlsCheckpoint(conversationId, expectedRevision, bytes, companion);
        },
