...
export class MlsWorkerClient {
    ...
    initialize(conversationId: ConversationId, memberNumber: MemberNumber): Promise<void> {
        return this.exclusive(conversationId, async () => {
            ...
            try {
                ...
                if (!adoptedCompanion) companion?.fill(0);
            }
        });
    }
    ...
    createApplicationMessage(conversationId: ConversationId, payload: Uint8Array): Promise<Uint8Array> {
        return this.mutate(
            conversationId,
    ...
    close(): void {
        ...
        for (const session of this.sessions.values()) session.companion?.fill(0);
        this.sessions.clear();
        this.queues.clear();
    }
